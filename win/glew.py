...
def get_glew(cache, build_path, arch, package, output, download_only=False):
    url = ('http://jaist.dl.sourceforge.net/project/glew/glew/'
           '{}/glew-{}-win32.zip'.
    ...
    z = base_dir = join(
        build_path, package, list(listdir(join(build_path, package)))[0])

    data = []
    for fname in glob(join(z, 'include', 'GL', '*')):
        data.append((
            fname, fname.replace(z, '').strip(sep), join('include', 'GL'),
            True))

    target = 'x64' if arch == 'x64' else 'Win32'

    data.append((
        join(z, 'lib', 'Release', target, 'glew32.lib'),
        join('lib', 'glew32.lib'), 'libs', True))

    data.append((
        join(z, 'bin', 'Release', target, 'glew32.dll'),
        join('bin', 'glew32.dll'),
